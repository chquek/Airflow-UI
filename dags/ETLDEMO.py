...
dag_kwargs = AttrDict ( {'owner_links': AttrDict({'airflow': 'https://airflow.apache.org'}), 'concurrency': 8, 'catchup': False, 'tags': ['demo', 'etl', 'transform', 'filter', 'database', 'excel', 'demo', 'triggerdag'], 'doc_md': '# Demonstrates using airflow for ETL\n\n- extract from employee table from DB2\n- use the data to write to excel\n- generate a PDF file\n- insert into table in mysql\n\n'}  )
