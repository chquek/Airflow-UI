...
tasks = [{'id': 247, 'pid': 31, 'name': 'Task1', 'operator': 'Bash', 'body': {'depend': [], 'detail': {'baseop': 'from airflow.operators.bash import BashOperator', 'bash_command': 'date > /workarea/file1.txt', 'callback': {'on_success_callback': 'udm.Callback.Task.task_success_callback', 'on_failure_callback': 'udm.Callback.Task.task_failure_callback', 'on_retry_callback': 'udm.Callback.Task.task_retry_callback', 'on_execute_callback': 'udm.Callback.Task.task_execute_callback'}, 'dataset': 'file:///workarea/file1.txt', 'doc_md': 'Markdown for BASH', 'runnable': "BashOperator ( task_id=taskname, ** ( t.kwargs ( [ 'bash_command' ] ) ) ,  default_args=task_opargs , doc_md=doc_md , **callback , **dataset , **other_args , params=dagparams )", 'task_opargs': {}, 'task_params': {}, 'trigger': 'all_success', 'other_args': {'trigger_rule': 'all_success'}}}, 'bfield': None}]
dagparams = AttrDict({'dagparam': 123})

dag_kwargs = AttrDict ( {'owner_links': AttrDict({}), 'concurrency': 8, 'catchup': True, 'tags': ['dataset', 'schedule', 'feature'], 'doc_md': '# Header - DAG Markdown notes\n\n- See this for : [default arg options](https://airflow.apache.org/docs/apache-airflow/1.10.8/tutorial.html#default-arguments)\n'}  )
default_args = AttrDict({'owner': 'anybody', 'start_date_old': '2023-07-24T18:13', 'start_date': '2023-09-10T00:00'})

'''
'''
dag_kwargs.owner_links = AttrDict({})
dag_kwargs.tags = ['dataset', 'schedule', 'feature']
