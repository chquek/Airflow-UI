...
dbobj = MsSqlHook(mssql_conn_id="mssql_kinisi")
logging.error ( dbobj )

tasks = [{'id': 358, 'pid': 89, 'name': 'FindTables', 'operator': 'MSSQL', 'body': {'depend': [], 'detail': {'baseop': 'from airflow.operators.python import PythonOperator', 'callback': {}, 'dataset': None, 'doc_md': None, 'format': 'dict', 'operation': 'Select', 'provider': 'mysql', 'python_callable': 'udm.SQL.run', 'runnable': 'PythonOperator ( task_id=taskname, python_callable = t.name_to_module( t.get("python_callable")  ) ,  default_args=task_opargs , doc_md=doc_md , **callback , **dataset , **other_args , params=dagparams )', 'sql': 'select * from Country', 'task_opargs': {}, 'task_params': {}, 'trigger': 'all_success', 'mssql_conn_id': 'mssql_kinisi', 'other_args': {'trigger_rule': 'all_success'}}}, 'bfield': None}]
dagparams = AttrDict({})

dag_kwargs = AttrDict ( {'owner_links': AttrDict({}), 'concurrency': 8, 'catchup': False, 'tags': ['mssql'], 'doc_md': '# Header - DAG Markdown notes\n\n- See this for : [default arg options](https://airflow.apache.org/docs/apache-airflow/1.10.8/tutorial.html#default-arguments)\n'}  )
default_args = AttrDict({'owner': 'anybody', 'start_date_old': '2023-08-08T15:14', 'start_date': '2023-08-01T15:14'})

'''
'''
dag_kwargs.owner_links = AttrDict({})
dag_kwargs.tags = ['mssql']
