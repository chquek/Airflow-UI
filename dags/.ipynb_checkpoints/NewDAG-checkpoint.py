...
tasks = [{'id': 343, 'pid': 88, 'name': 'NEWTASK', 'operator': 'Python', 'body': {'depend': [], 'detail': {'baseop': 'from airflow.operators.python import PythonOperator', 'callback': {}, 'code': 'udm.Template.info', 'dataset': None, 'doc_md': None, 'python_callable': 'udm.Template.info', 'runnable': 'PythonOperator ( task_id=taskname, python_callable = t.run  ,  default_args=task_opargs , doc_md=doc_md , **callback , **dataset , **other_args , params=dagparams )', 'task_opargs': {}, 'task_params': {}, 'trigger': 'all_success', 'other_args': {'trigger_rule': 'all_success'}}}, 'bfield': None}]
...
default_args = AttrDict({'owner': 'anybody', 'start_date_old': '2023-08-07T15:26', 'start_date': '2023-08-01T15:26'})
...
with DAG(DAG_ID, default_args=default_args, **dag_kwargs , on_success_callback = success_callback , on_failure_callback = failure_callback ) as dag:
    ...
    for idx,task in enumerate(tasks) :
        ...
        if t.id() == 343 :
            mod = t.name_to_module(t.get("python_callable"))
